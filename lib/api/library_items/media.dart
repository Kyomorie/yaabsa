...
abstract class Media with _$Media {
  ...
  const factory Media({PodcastMedia? podcastMedia, BookMedia? bookMedia}) =
      _Media;
  ...
  List<String>? get authors => podcastMedia != null
      ...
      : bookMedia!.metadata.authors?.map((e) => e.name).toList() ??
            bookMedia!.metadata.authorName?.split(',');
  ...
  double duration({String? episodeId}) {
    if (podcastMedia != null) {
      if (episodeId != null) {
        final episode = podcastMedia!.episodes
      ...
      return episodes.fold<double>(
        0,
        (total, episode) => total + (episode.audioFile?.duration ?? 0),
      );
    } else {
      ...
      return audioFiles.fold<double>(
        0,
        (total, audioFile) => total + (audioFile.duration ?? 0),
      );
    }
  }
}
