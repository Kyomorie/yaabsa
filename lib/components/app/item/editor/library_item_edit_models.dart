...
class LibraryItemEditorSeriesEntry {
  ...
  LibraryItemEditorSeriesEntry copyWith({
    String? id,
    String? name,
    String? sequence,
  }) {
...
class LibraryItemEditorDraft {
  ...
  factory LibraryItemEditorDraft.fromLibraryItem(
    LibraryItem item, {
    String? descriptionHtmlOverride,
  }) {
    ...
    return LibraryItemEditorDraft(
      ...
      authors: (bookMetadata?.authors ?? const <Author>[])
          .map(
            (author) => LibraryItemEditorNamedEntity(
              id: _normalizedText(author.id).isNotEmpty
                  ? _normalizedText(author.id)
                  : _normalizedText(author.name),
      ...
      narrators: _normalizeStringList(
        bookMetadata?.narrators ?? const <String>[],
      ),
      series: (bookMetadata?.series ?? const <Series>[])
          .map(
            (entry) => LibraryItemEditorSeriesEntry(
              id: _normalizedText(entry.id).isNotEmpty
                  ? _normalizedText(entry.id)
                  : _normalizedText(entry.name),
          ...
          .where((entry) => entry.name.isNotEmpty)
          .toList(growable: false),
      genres: _normalizeStringList(
        bookMetadata?.genres ?? podcastMetadata?.genres ?? const <String>[],
      ),
      tags: _normalizeStringList(
        item.media?.bookMedia?.tags ??
      ...
      descriptionHtml:
          ...
          (bookMetadata?.description ?? podcastMetadata?.description ?? ''),
      isbn: (bookMetadata?.isbn ?? '').trim(),
      asin: (bookMetadata?.asin ?? '').trim(),
      language: (bookMetadata?.language ?? podcastMetadata?.language ?? '')
          .trim(),
...
LibraryItemEditorDiff buildLibraryItemEditorDiff({
  ...
  void setMetadata(
    UpdateLibraryItemMediaMetadataPatch Function(
      UpdateLibraryItemMediaMetadataPatch current,
    )
    updater,
  ) {
  ...
  if (initial.isPodcast) {
    if (_normalizedText(initial.podcastAuthor) !=
        _normalizedText(current.podcastAuthor)) {
      setMetadata(
        (m) => m.copyWith(author: _normalizedText(current.podcastAuthor)),
      );
    }

    if (_normalizedText(initial.releaseDate) !=
        _normalizedText(current.releaseDate)) {
      setMetadata(
        (m) => m.copyWith(releaseDate: _normalizedText(current.releaseDate)),
      );
    }
    ...
    if (_normalizedText(initial.itunesId) !=
        _normalizedText(current.itunesId)) {
      setMetadata(
        (m) => m.copyWith(itunesId: _normalizedText(current.itunesId)),
      );
    }

    if (_normalizedText(initial.podcastType) !=
        _normalizedText(current.podcastType)) {
      setMetadata(
        (m) => m.copyWith(type: _normalizedText(current.podcastType)),
      );
    }
  } else {
    if (_normalizedText(initial.subtitle) !=
        _normalizedText(current.subtitle)) {
      setMetadata(
        (m) => m.copyWith(subtitle: _normalizedText(current.subtitle)),
      );
    }
          ...
          .map(
            (entry) => Series(
              id: _normalizedText(entry.id).isNotEmpty
                  ...
                  : _normalizedText(entry.name),
              name: _normalizedText(entry.name),
              sequence: _normalizedText(entry.sequence).isEmpty
                  ? null
                  : _normalizedText(entry.sequence),
            ),
          )
          .where((entry) => _normalizedText(entry.name).isNotEmpty)
          .toList(growable: false);
    ...
    if (_normalizedText(initial.publishedYear) !=
        _normalizedText(current.publishedYear)) {
      setMetadata(
        (m) =>
            m.copyWith(publishedYear: _normalizedText(current.publishedYear)),
      );
    }

    if (_normalizedText(initial.publisher) !=
        _normalizedText(current.publisher)) {
      setMetadata(
        (m) => m.copyWith(publisher: _normalizedText(current.publisher)),
      );
    }
...
bool _sameNamedEntities(
  List<LibraryItemEditorNamedEntity> a,
  List<LibraryItemEditorNamedEntity> b,
) {
...
bool _sameSeries(
  List<LibraryItemEditorSeriesEntry> a,
  List<LibraryItemEditorSeriesEntry> b,
) {
  if (a.length != b.length) {
    return false;
  }
