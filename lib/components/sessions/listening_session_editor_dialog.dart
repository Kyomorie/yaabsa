...
class _ListeningSessionEditorDialog extends StatefulWidget {
  ...
  @override
  State<_ListeningSessionEditorDialog> createState() =>
      _ListeningSessionEditorDialogState();
}

class _ListeningSessionEditorDialogState
    extends State<_ListeningSessionEditorDialog> {
  ...
  Future<void> _save() async {
    ...
    if (_sessionDate != null) {
      ...
      if (newStartedAt != null) {
        final originalStarted = DateTime.fromMillisecondsSinceEpoch(
          newStartedAt,
        );
      ...
      if (newUpdatedAt != null) {
        final originalUpdated = DateTime.fromMillisecondsSinceEpoch(
          newUpdatedAt,
        );
          ...
          child: Column(
            ...
            children: [
              ...
              if (userLabel != null && userLabel.isNotEmpty)
                _buildSummaryTile('User', userLabel),
              _buildSummaryTile(
                'Date',
                formatDateTimeLabel(_sessionDate),
                onEdit: widget.canEdit
                    ? () => _editTimestamp(
                        _sessionDate?.millisecondsSinceEpoch,
                        (val) {
                          if (val != null) {
                            setState(
                              () => _sessionDate =
              ...
              _buildSummaryTile(
                ...
                onEdit: widget.canEdit
                    ? () => _editDuration(
                        'Start',
                        _startSeconds,
                        (val) => setState(() => _startSeconds = val),
                      )
                    : null,
              ),
              _buildSummaryTile(
              ...
              if (mediaPlayer != null && mediaPlayer.isNotEmpty)
                _buildSummaryTile('Player', mediaPlayer),
              if (serverVersion != null && serverVersion.isNotEmpty)
                _buildSummaryTile('Server Version', serverVersion),
              ...
              if (episodeId != null && episodeId.isNotEmpty)
                _buildSummaryTile('Episode ID', episodeId),
