...
class _CurrentUserListeningSessionsTabState
  ...
  Future<ListeningSessionsPage> _getListeningSessionsPage({
    required ABSApi api,
    ...
    try {
      ...
      if (statusCode == 404 || statusCode == 405) {
        final fallbackResponse = await api.getMeApi().getUserListeningSessions(
          userId,
          page: page,
          itemsPerPage: _itemsPerPage,
        );
  ...
  Future<void> _loadSessions() async {
    ...
    try {
      ...
      logger(
        'Failed to load user listening sessions: $error\n$stackTrace',
        tag: 'CurrentUserListeningSessionsTab',
  ...
  Future<void> _bulkDeleteSelectedSessions(User currentUser) async {
    ...
    for (final session in selectedSessions) {
      try {
        ...
        logger(
          'Failed deleting session ${session.id}: $error\n$stackTrace',
          tag: 'CurrentUserListeningSessionsTab',
          level: InfoLevel.error,
        );
        failedSessionIds.add(session.id);
