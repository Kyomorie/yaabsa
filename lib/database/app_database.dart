...
class AppDatabase extends _$AppDatabase {
  ...
  Future<bool> acknowledgeReplayedSync(StoredSyncEntry replayed) {
    return transaction(() async {
      final current =
          await (select(storedSyncs)
                ..where((tbl) => tbl.sessionId.equals(replayed.sessionId)))
              .getSingleOrNull();
      if (current == null) {
        return true;
      }
      ...
      if (unchangedSnapshot) {
        await (delete(
          storedSyncs,
        )..where((tbl) => tbl.sessionId.equals(replayed.sessionId))).go();
        return true;
      }
      ...
      await (update(
        storedSyncs,
      )..where((tbl) => tbl.sessionId.equals(replayed.sessionId))).write(
        StoredSyncsCompanion(
          timeListened: Value(
            remainingListened < epsilon ? 0.0 : remainingListened,
          ),
        ),
      );
      return true;
