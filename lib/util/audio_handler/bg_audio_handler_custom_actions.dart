...
extension _BGAudioHandlerCustomActions on BGAudioHandler {
  Future<Map<String, dynamic>?> _handleCustomAction(
    String name, {
    ...
    if (name == _widgetCustomActionPlayLast) {
      ...
      logger(
        'Widget custom action: play last (started=$started)',
        tag: 'AudioHandler',
    ...
    if (name == _androidAutoCustomActionRewind) {
      ...
      await rewind();
      logger(
        'Android Auto custom action: rewind',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
    ...
    if (name == _androidAutoCustomActionFastForward) {
      ...
      await fastForward();
      logger(
        'Android Auto custom action: fast forward',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
      return <String, dynamic>{'handled': true};
    ...
    if (name == _androidAutoCustomActionSpeed) {
      ...
      final currentIndex = steps.indexWhere(
        (value) => (value - currentSpeed).abs() < 0.001,
      );
      ...
      logger(
        'Android Auto custom action: speed changed to $nextSpeed',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
    ...
    if (name == _androidAutoCustomActionMoreMenu) {
      _setAndroidAutoMoreMenuVisible(
        true,
        autoCloseAfter: const Duration(seconds: 4),
      );

      logger(
        'Android Auto more menu opened',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
    ...
    if (name == _androidAutoCustomActionMoreClose) {
      ...
      logger(
        'Android Auto custom action: more menu closed',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
    ...
    if (name == _androidAutoCustomActionStop) {
      ...
      await stop();
      logger(
        'Android Auto custom action: stop invoked',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
    ...
    if (name == 'custom.switch_page') {
      ...
      logger(
        'Media notification custom action: switch page',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
    ...
    if (name == 'custom.skip_next') {
      await skipToNext();
      logger(
        'Media notification custom action: skip to next',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
    ...
    if (name == 'custom.skip_previous') {
      ...
      logger(
        'Media notification custom action: skip to previous',
        tag: 'AudioHandler',
        level: InfoLevel.info,
      );
